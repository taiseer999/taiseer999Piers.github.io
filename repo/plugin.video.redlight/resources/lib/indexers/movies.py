...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			try:
				import traceback
				kodi_utils.logger('RedLight List Error', '%s: %s\n%s' % (self.action, e, traceback.format_exc()))
			except: pass
	...
	def build_movie_content(self, _position, _id, dbcon=None):
		try:
			...
			try:
				if not getattr(self, '_item_err_logged', False):
					self._item_err_logged = True
					import traceback
					kodi_utils.logger('RedLight Item Error', 'id=%r: %s\n%s' % (_id, e, traceback.format_exc()))
			except: pass
