...
__all__ = ["lookup", "database", "add_locations", "all_locations", "search_locations"]
...
def search_locations(name: str, count: int = 10) -> List[LocationInfo]:
    """Search for locations by name.

    First searches the built-in local database (case-insensitive substring
    match on the city name), then falls back to the Open-Meteo Geocoding API
    when connected to the internet.  The Open-Meteo API is free, requires no
    API key, and returns the IANA timezone for each result.

    Args:
        name:  The city / location name to search for
        count: Maximum number of results to return (default 10)

    Returns:
        A list of :class:`~astral.LocationInfo` objects with ``name``,
        ``region``, ``timezone``, ``latitude`` and ``longitude`` populated.
        Returns an empty list if nothing is found and the network is
        unavailable.

    Example::

        from astral.geocoder import search_locations
        for loc in search_locations("Berlin"):
            print(loc.name, loc.timezone, loc.latitude, loc.longitude)
    """
    results: List[LocationInfo] = []

    # --- 1. Search the built-in local database (offline, instant) ----------
    key = _sanitize_key(name)
    db = database()
    for _group_key, group_info in db.items():
        for location_name, location_list in group_info.items():
            if key in location_name:
                results.extend(location_list)
    ...
    # --- 2. Fallback: Open-Meteo Geocoding API (online) --------------------
    # https://geocoding-api.open-meteo.com — free, no API key required.
    try:
        ...
        url = (
            ...
            "?name=%s&count=%d&language=en&format=json" % (quote(name), count)
        )
        req = Request(url, headers={"User-Agent": "Kodi/script.module.astral"})
        ...
        pass  # réseau indisponible ou erreur API — on retourne ce qu'on a
